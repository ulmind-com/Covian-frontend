...
export function FeaturedCompanies() {
  ...
  return (
    <section className="py-24 bg-white rounded-t-[2rem] shadow-[0_-20px_60px_rgba(0,0,0,0.15)] relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#003A70]/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#00A86B]/5 rounded-full blur-[100px] pointer-events-none" />
      
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1E88E5]/10 text-[#1E88E5] text-sm font-bold mb-6 tracking-wide">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {displayCompanies.slice(0, 6).map((company, i) => (
              ...
              >
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#1E88E5] to-[#00A86B] rounded-t-3xl opacity-0 group-hover:opacity-100 transition-opacity" />
                
                <div>
                  <div className="flex justify-between items-start mb-6">
                    <div className="w-16 h-16 rounded-2xl bg-gray-50/50 border border-gray-100 flex items-center justify-center shadow-inner group-hover:bg-[#1E88E5]/5 transition-colors">
                      <Building className="w-8 h-8 text-[#003A70]/40 group-hover:text-[#1E88E5] transition-colors" />
                    </div>
                    <span className="px-3 py-1 bg-[#003A70]/5 text-[#003A70] text-xs font-bold rounded-full uppercase tracking-wider group-hover:bg-[#003A70] group-hover:text-white transition-colors">
                      {company.industry || "Enterprise"}
                    </span>
                  </div>
                  
                  <h3 className="text-2xl font-bold text-[#003A70] mb-3">{company.name}</h3>
                ...
                <div className="mt-8 flex items-center justify-between pt-6 border-t border-gray-100">
                  <div className="flex items-center text-sm font-bold text-[#00A86B]">
                    <Users className="w-4 h-4 mr-1.5" /> Actively Hiring
                  </div>
                  <Button variant="ghost" size="sm" className="text-[#1E88E5] hover:text-white hover:bg-[#1E88E5] rounded-full font-semibold transition-colors">
