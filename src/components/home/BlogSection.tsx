...
export function BlogSection() {
  ...
  return (
    <section className="py-24 bg-white relative overflow-hidden">
      <div className="absolute top-1/2 left-0 w-[500px] h-[500px] bg-[#00A86B]/5 rounded-full blur-[120px] pointer-events-none -translate-y-1/2" />
      
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#00A86B]/10 text-[#00A86B] text-sm font-bold mb-6 tracking-wide">
          ...
          <Link href="/blog">
            <Button className="bg-[#003A70] hover:bg-[#1E88E5] text-white rounded-full px-8 h-12 shadow-[0_10px_30px_rgba(4,43,107,0.2)] font-bold transition-all hover:pr-6 group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {displayBlogs.map((blog: any, i: number) => (
              <Link key={blog.id || blog.slug || i} href={`/blog/${blog.slug}`}>
                ...
                >
                  <div className="w-full h-48 relative overflow-hidden bg-gradient-to-br from-[#1E88E5] to-[#003A70]">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#003A70]/80 to-transparent opacity-80" />
                  ...
                  <div className="p-8 flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center gap-4 text-xs font-bold text-slate-400 mb-4">
                        <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-[#1E88E5]" /> {formatDate(blog.created_at)}</span>
                        <span className="flex items-center gap-1.5"><User className="w-4 h-4 text-[#00A86B]" /> {blog.author}</span>
                      </div>
                      <h3 className="text-2xl font-bold text-[#003A70] mb-4 group-hover:text-[#1E88E5] transition-colors leading-snug line-clamp-3">
                        {blog.title}
                      </h3>
                    </div>
                    <div className="flex items-center text-[#1E88E5] font-bold text-sm group-hover:translate-x-2 transition-transform mt-6 pt-6 border-t border-gray-100">
